...
async function main() {
  ...
  try {
    ...
    let nb = await req('GET', '/notebook', null, 'coach');
    ...
    const hm = await req('GET', '/', null, 'coach');
    ...
    // View-as the Bobby-org player: full visibility kept.
    // (Exit the college preview first — view-as mode blocks POSTs by design.)
    await req('POST', '/coach/view-as/exit', P({}), 'coach');
    await req('POST', '/coach/view-as', P({ id: String(bobbyKid) }), 'coach');
    nb = await req('GET', '/notebook', null, 'coach');
    check('bobby-org notebook keeps Full entry', nb.text.includes('Full entry'));
    check('bobby-org notebook keeps diary words', nb.text.includes('PRIVATE-DIARY-BOBBY'));
    await req('POST', '/coach/view-as/exit', P({}), 'coach');

    // Per-player coach page.
    let cp = await req('GET', '/coach/user/' + encodeURIComponent('collegekid@test.com'), null, 'coach');
    ...
    check('bobby coach page keeps chat history', cp.text.includes('Chat history'));
    check('bobby coach page keeps diary words', cp.text.includes('PRIVATE-DIARY-BOBBY'));

    // Coach home latest feed: college card restricted per-row.
    const home = await req('GET', '/coach', null, 'coach');
    ...
    // Hitter list: streak + frequency visible.
    ...
    // Export: college rows redacted, Bobby rows intact.
    const ex = await req('GET', '/coach/export', null, 'coach');
    check('export 200', ex.status === 200);
    const data = JSON.parse(ex.text);
    const colRow = data.checkins.find((r) => r.email === 'collegekid@test.com');
    const bobRow = data.checkins.find((r) => r.email === 'bobbykid@test.com');
