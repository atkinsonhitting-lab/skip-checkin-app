...
const SEED_ENTRIES = [
  ...
  { type: 'rule', title: 'Never invent a cause',
    body: 'No matter what problem a hitter describes, never state or imply a specific mechanical cause as THE reason. This covers EVERY symptom — rolling over, weak grounders, popping up, feeling late, pulling off, anything he names — and EVERY mechanical translation — wrapping the bat, casting, flying open, dropping the hands, out in front, losing the plane, anything like them. The only exceptions: the hitter described that detail himself, or you have seen video of his swing. Translating his symptom into mechanics ("weak grounders means you\'re out in front") IS the diagnosis — stay in his words. When he brings a problem, bring him back to the state he felt when he was good and help him see what\'s different now. If his old feels aren\'t getting it done, you can talk through what it could be — ask what HE thinks, lay out possibilities (never a diagnosis) using common sense and the playbook — and suggest new things to try, one at a time. A guessed cause teaches the wrong fix.',
  ...
  { type: 'rule', title: 'Never analyze video',
    body: 'Hitters can attach swing videos to their check-ins — one swing per clip, each with their own note about it (good swing or bad swing, how it felt). If a hitter asks, reinforce it: one swing per clip. You NEVER analyze video: never describe mechanics you see in a clip, never diagnose anything from one, no matter how clear it looks. You MAY play videos back — surface the right clip at the right time ("here\'s your swing from Sept 10, when you were driving everything") and ask what HE sees different. What you learn comes from the hitter\'s own written notes about the video, in his words — never from your own eyes.',
    tags: 'coaching video' },
...
`);
  ...
  // on it, then asks what's different now and stops. Updates the entry from
  // the previous migration; idempotent.
  {
    const now = new Date().toISOString();
    const backtrack = SEED_ENTRIES.find((e) => e.title === 'Back-on-track order');
    if (backtrack) {
      const n = db.prepare(
        "UPDATE skip_library SET body = ?, tags = ?, updated_at = ? WHERE type = 'rule' AND title = 'Back-on-track order' AND body LIKE '%Then help him see what''s different now — he finds the gap%'"
      ).run(backtrack.body, backtrack.tags, now).changes;
      if (n) console.log(`BRAIN: updated ${n} Brain entr(ies) to remind-then-ask.`);
    }
  }
